                    ...
                    href={`${routes.ItemResidential}/${item._id}`}>
                        <div className='campoRustico__list__item__text'>
                            ...
                            <ul className='campoRustico__list__item__text__characteristics'>
                                ...
                                :null}
                                {item.quality.bathrooms !== 0 ?
                                    <li><span><mage width={15} height={12} src={banera} alt='baños'/></span>{item.quality.bathrooms}</li>
                                :null}
                                {item.quality.bedrooms !== 0 ?
                                    <li><span><Image width={18} height={12} src={habit} alt='habitaciones'/></span>{item.quality.bedrooms}</li>
                                :null}
    ...
    useEffect(() => {
        ...
        for(let i = 0; i< pageCount; i++){
            if(URLwithoutPage.length !== 0) {
                ...
                elements.push(
                <li key={i} className={i + 1 === parseInt(splitedLocation[4]) ? 'campoRustico__pagination__list__item currentPage' : 'campoRustico__pagination__list__item'}><a href={`/costa/${i + 1}?${URL}&page=${i+1}`}>{i + 1}</a></li>
                )
            }else{
                elements.push(
                    <li key={i} className={i + 1 === parseInt(splitedLocation[4]) ? 'campoRustico__pagination__list__item currentPage' : 'campoRustico__pagination__list__item'}><a href={`/costa/${i + 1}?page=${i+1}`}>{i + 1}</a></li>
                    )
            }
