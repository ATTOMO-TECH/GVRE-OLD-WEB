...
import { getConsultants, getPatrimonialItem } from '../../api-requests/requests.js'
...
const QRGenerator = dynamic(() => import("../../components/QRgenerator/QRgenerator"), {
    ssr: false
  });
const DownLoadBuildingSheet = dynamic(() => import("../../components/DownLoadBuildingSheet/DownLoadBuildingSheet.js"), {
    ssr: false
  });

const DownLoadBuildingDrawings = dynamic(() => import("../../components/DownloadDrawings/DownloadDrawings.js"), {
  ssr: false
});
...
export default function PatrimonialItem({list, currentConsultant}){
    ...
    useEffect(() => {
                    ...
                    Geocode.fromAddress(`${state.adDirection.address.street}${state.adDirection.address.directionNumber}, ${state.adDirection.city}`).then(
                        (response) => {
    ...
    return(
        ...
        <div>
            <Header/>
            {state?.featuredDrawings !== undefined && state?.featuredDrawings !== false ?
                <Carousel 
                ...
                autoFocus={true}
            >
                <Image width={1486} height={862} className='patrimonialItem__carousel__images custom-objetc-fit' src={state.images.main} alt={state.title} loading="lazy"/>
                {state.images.others.map((image)=> (
                    <Image width={1486} height={862} className='patrimonialItem__carousel__images custom-objetc-fit' key={state._id} src={image} alt={state.title} loading="lazy"/>
                ))}
                {state.images.blueprint.map((image)=> (
                        <Image width={1486} height={862} className='patrimonialItem__carousel__images custom-objetc-fit' key={image} src={image} alt={state.title}/>
                    ))}
            </Carousel> :
                <Carousel 
                className='patrimonialItem__carousel'
            ...
            {viewFullScreen === true ? 
                <div className='patrimonialItem__fullScreen'>
                    <button onClick={toggleFullScreen} className='patrimonialItem__fullScreen__close'><Image width={14} height={14} src={closeFullScreen} alt='close full screen'/></button>
                    <Carousel 
                        ...
                        autoFocus={true}
                    >
                        <Image width={1486} height={862} className='carouselImages' src={state.images.main} alt={state.title} loading="lazy"/>
                        {state.images.others.map((image)=> (
                            <Image width={1486} height={862} className='carouselImages' key={state._id} src={image} alt={state.title} loading="lazy"/>
                        ))}
                        {state.featuredDrawings && state.images.blueprint.length !==0 ? state.images.blueprint.map((image)=> {
                                //console.log('ruta:', image)
                                return (
                                <Image width={1486} height={862} className='carouselImages' key={image} src={image} alt={state.title}/>
                            )
                            }): null}
                    </Carousel>
                </div>
            ...
            {viewMap === true ?
                <div className='patrimonialItem__map'>
                    <button onClick={toggleMap} className='patrimonialItem__map__close'><Image width={14} height={14} src={closeFullScreen} alt='close full screen'/></button>
                    <Carousel 
                        className='patrimonialItem__fullScreen__carousel'
                        showArrows={true}
                        showThumbs={false}
                    ...
                    >
                        {state.images.blueprint.map((image)=> (
                            <Image width={1486} height={862} className='carouselImages custom-map-size' key={image} src={image} alt={state.title}/>
                        ))}
                    </Carousel>
                </div>
            :null
            ...
            <div className='patrimonialItem__description'>
                <div className={state.expensesIncluded !== 0 && state.monthlyRent !== 0 && state.expenses !== 0 ?'patrimonialItem__description__principal' : 'patrimonialItem__description__principalEmpty'}>
                    <Image width={22} height={20} onClick={toggleFullScreen} src={fullScreen} alt='full screen'/>
                    ...
                    <h3 className='patrimonialItem__description__web__subtitle' >{state.webSubtitle}</h3>
                </div>
                {state.adType.map((item, i) => 
                    item === 'Alquiler' ? 
                    <div key={i} className={state.expensesIncluded !== 0 && state.monthlyRent !== 0 && state.expenses !== 0 ?'patrimonialItem__description__rent' : 'patrimonialItem__description__rentEmpty'}>
                ...
                {state.description.web !== '' ? 
                    ...
                    {state.images.media && state.images.media !== '' ? <DisplayVideo videoUrl={state.images.media} portraitImage={state.images.main} /> : null }

                        <h2>Descripción</h2>
                        {/* <p>{state.description.web}</p> */}
                        <textarea spellCheck="false" disabled value={state.description.web}/>
                ...
                <div className='patrimonialItem__description__distribution'>
                    ...
                    <textarea spellCheck="false" disabled  value={state.description.distribution}/>
                    :null}
                   <div className='patrimonialItem__description__distribution__buttons'>
                            {state !== null && id !== undefined && state.images.blueprint.length!== 0 &&
                                <DownLoadBuildingDrawings state={state}></DownLoadBuildingDrawings>
                            }
                            {(state?.featuredDrawings === undefined || !state?.featuredDrawings) &&  state.images.blueprint.length !== 0 ?
                                                <button onClick={toggleMap}>Ver plano</button>
                                        :null
                            }  
                             {/* SI CAMBIA LA URL DE RESIDENTIALITEM HAY QUE CAMBIAR ESTE PATH TAMBIEN */}
                             {state !== null && id !== undefined && <QRGenerator urlString={`https://gvre.es/patrimonialItem/${state._id}`}/> }
                            {state !== null && id !== undefined &&
                                <DownLoadBuildingSheet state={state} currentConsultant={currentConsultant}/> 
                            }         
                   </div>           
                </div>
                <div className='patrimonialItem__description__numbers'>
                    ...
                    <div>
                        ...
                        :null}
                        {state.trashFee.trashFeeValue!==0 && state.trashFee.trashFeeShowOnWeb===true ? 
                            <div className='patrimonialItem__description__numbers__bed'>
                                <p className='patrimonialItem__description__numbers__bed__data'>{state.trashFee.trashFeeValue}</p>
                ...
                <div className='patrimonialItem__description__extras'>
                    {state.quality.others.accessControl === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Control de accesos</p> : null}
                    {state.quality.others.airConditioning === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Aire acondicionado</p> : null}
                    {state.quality.others.centralHeating === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Calefacción central</p> : null}
                    {state.quality.others.centralVacuum === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Aspiración centralizada</p> : null}
                    {state.quality.others.dumbwaiter === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Montaplatos</p> : null}
                    {state.quality.others.falseCeiling === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Falso techo</p> : null}
                    {state.quality.others.freeHeight === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Altura libre 2.5m</p> : null}
                    {state.quality.others.fullHoursSecurity === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Seguridad 24h</p> : null}
                    {state.quality.others.garage === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Garaje</p> : null}
                    {state.quality.others.gunRack === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Armero</p> : null}
                    {state.quality.others.homeAutomation === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Domótica</p> : null}
                    {state.quality.others.indoorAlarm === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Alarma interior</p> : null}
                    {state.quality.others.lift === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Ascensor</p> : null}
                    {state.quality.others.liftTruck === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Montacargas</p> : null}
                    {state.quality.others.outdoorAlarm === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Alarma perimetral</p> : null}
                    {state.quality.others.padelCourt === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Pista de pádel</p> : null}
                    {state.quality.others.qualityBathrooms === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Baños</p> : null}
                    {state.quality.others.smokeOutlet === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Salida de humos</p> : null}
                    {state.quality.others.storage === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Trastero</p> : null}
                    {state.quality.others.strongBox === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Caja fuerte</p> : null}
                    {state.quality.others.subFloorHeating === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Suelo radiante</p> : null}
                    {state.quality.others.swimmingPool === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Piscina</p> : null}
                    {state.quality.others.tennisCourt === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Pista de tenis</p> : null}
                    {state.quality.others.terrace === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Terraza</p> : null}
                    {state.quality.others.well === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Pozo</p> : null}
                    {state.quality.others.raisedFloor === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Suelo técnico</p> : null}
                    {state.quality.others.furnished === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Amueblada</p> : null}
                    {state.quality.others.implanted === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Implantada</p> : null}
                    {state.quality.others.separateEntrance === true ? <p> <Image width={20} height={20} src={check} alt='check'/> Entrada independiente</p> : null}
                </div>
                        ...
                        {state.quality.subway!=='' ? 
                            <div className='patrimonialItem__description__transport__bed'>
                                <div>
                                    ...
                                    <p>Metro -</p>
                                    <p className='patrimonialItem__description__transport__bed__data '>{state.quality.subway.replace(',', ' -')}</p>
                                </div>
                            </div>
                        :null}
                        {state.quality.bus!=='' ? 
                            <div className='patrimonialItem__description__transport__bed'>
                                <div>
                                    ...
                                    <p>Autobús</p>
                                    <p className='patrimonialItem__description__transport__bed__data '>{state.quality.bus}</p>
                                </div>
                            </div>
                        :null}
                    </div>
                    ...
                    <div className='patrimonialItem__description__owner'>
                        {consultant !== undefined ?
                            <div key={consultant._id} className='patrimonialItem__description__owner__details'>
                                <Image width={size < 768 ? 558 : size < 1350 ? 345 : 200} height={size < 768 ? 240 : size < 1350 ? 232 : 140} src={consultant.avatar} alt={consultant.fullName}/>
                        ...
                        {viewForm === true ? 
                            ...
                            >
                                {({errors, touched, isSubmitting}) => (
                                    ...
                                    <button className='patrimonialItem__description__form__button' type='submit' disabled={isSubmitting}>Enviar <span><Image width={35} height={16} src={send} alt='enviar'/></span></button>
                                </Form>
                                )}
                            </Formik>
                            :
                ...
                <div className='goBack'>
                        <button className='goBack__button' onClick={goBack}>Volver al listado</button>
                </div>
        ...
        //console.log('query id',id)
            ...
            if(item !== null){
                ...
                if(buildingId === id){
                    ...
                    currentConsultant = itemConsultants.find(consultant => consultant._id === item.consultant)
                }
                return {
                    props: {
                        list, 
