...
import { getConsultants, getResidentialItem } from '../../api-requests/requests.js'; 
...
export default function ResidentialItem({list, currentConsultant}){
    ...
    return (
        ...
        {state?.images?
            <div>
                <Header/>
                {isLoading ? state?.featuredDrawings !== undefined && state?.featuredDrawings !== false ?
                <Carousel
                    ...
                    autoFocus={true}
                >
                    <Image width={1486} height={862} className='residentialItem__carousel__images custom-objetc-fit' src={state.images.main} alt={state.title} loading="lazy"/>
                    {state.images.others.map((image)=> (
                        <Image width={1486} height={862} className='residentialItem__carousel__images custom-objetc-fit' key={state._id} src={image} alt={state.title} loading="lazy"/>
                    ))}
                    {state.images.blueprint.map((image)=> (
                        <Image width={1486} height={862} className='residentialItem__carousel__images custom-objetc-fit' key={image} src={image} alt={state.title}/>
                    ))}
                ...
                {viewFullScreen === true ?
                    <div className='residentialItem__fullScreen'>
                        <button onClick={toggleFullScreen} className='residentialItem__fullScreen__close'><Image width={14} height={14} src={closeFullScreen} alt='close full screen'/></button>
                        <Carousel
                ...
                {viewMap === true ?
                    <div className='residentialItem__fullScreen'>
                        <button onClick={toggleMap} className='residentialItem__map__close'><Image width={14} height={14} src={closeFullScreen} alt='close full screen'/></button>
                        <Carousel
                            className='residentialItem__fullScreen__carousel'
                            showArrows={true}
                            showThumbs={false}
                ...
                <div className='residentialItem__description'>
                    <div className='residentialItem__description__principal'>
                        <button onClick={toggleFullScreen} ><Image width={22} height={20} src={fullScreen} alt='full screen'/></button>
                    ...
                    item === 'Alquiler' ?
                    <div key={i} className={state.expensesIncluded !== 0 && state.monthlyRent !== 0 && state.expenses !== 0 ?'residentialItem__description__rent' : 'residentialItem__description__rentEmpty'}>
                    ...
                    <div className='residentialItem__description__distribution'>
                        ...
                        <div className='residentialItem__description__distribution__buttons'>
                            ...
                            {state !== null && id !== undefined && <QRGenerator urlString={`https://gvre.es/residentialItem/${state._id}`}/> }
                                ...
                                >
                                    {({errors,isSubmitting, touched}) => (
                                        <Form ref={form} className='residentialItem__description__form__inputs'>
                                            ...
                                            <button className='residentialItem__description__form__button' type='submit' disabled={isSubmitting}>Enviar <span><Image width={35} height={16} src={send} alt='enviar'/></span></button>
                                        </Form>
                                    )}
                                </Formik>
                                :
